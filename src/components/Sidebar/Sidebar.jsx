...
useEffect(() => {
  const fetchTrustAndUser = async () => {
    ...
    // ✅ Load cached sidebar data first
    const cachedTrustName = localStorage.getItem("trustName");
    const cachedTrustLogo = localStorage.getItem("trustLogo");
    const cachedUserName = localStorage.getItem("sidebarUserName");

    if (cachedTrustName || cachedTrustLogo) {
      setTrust({
        name: cachedTrustName,
        logo: cachedTrustLogo,
      });
    }
    ...
    if (storedUser) {
      ...
      // ✅ Store user name
      localStorage.setItem("sidebarUserName", user.name || "");
      ...
      try {
        ...
        // ✅ Store trust data
        localStorage.setItem("trustName", trustData?.name || "");
        localStorage.setItem("trustLogo", trustData?.logo || "");
      } catch (error) {
        console.error("Error fetching trust:", error);
      }
    }
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem("userId");
      ...
      // ✅ Clear sidebar cached data
  localStorage.removeItem("trustName");
  localStorage.removeItem("trustLogo");
